...
export function PrayerTimesWidget({ initialData }: PrayerTimesWidgetProps) {
  ...
  const times = [
    ...
    { label: "Ashar", time: initialData.asr, icon: <Sun className="w-4 h-4" /> },
    { label: "Maghrib", time: initialData.maghrib, icon: <Moon className="w-4 h-4" /> },
    { label: "Isya", time: initialData.isha, icon: <Moon className="w-4 h-4" /> },
  ];

  return (
    // mt-8 memberi jarak aman dari teks judul di atasnya
    <div className="w-full max-w-4xl mx-auto mt-8 relative z-20 px-4 md:px-0">
      <Card className="border-none shadow-2xl bg-white overflow-hidden ring-1 ring-slate-900/5">
        ...
        {/* === ISI JADWAL (Flexbox Divide) === */}
        <CardContent className="p-0">
          {/* Teknik "Divide":
             - Mobile: divide-y (garis pemisah antar baris)
             - Desktop: divide-y-0 (hapus garis baris) & divide-x (tambah garis kolom)
          */}
